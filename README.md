...
| $p_i$                    | $p_i$ 为长度惩罚项。根据选手 answer 的长度 ${len}_{i}^{sub}$ 与标注文本块长度 ${len}_{i}^{std}$ 确定：<br> - 当 ${len}_{i}^{sub} \leq 1.5 {len}_{i}^{std}$ 时，该项为 `1`；<br> - 当 $1.5 {len}_{i}^{std} < {len}_{i}^{sub} \leq 2.5 {len}_{i}^{std}$ 时，该项为 `0.9`；<br> - 当 ${len}_{i}^{sub} > 2.5 {len}_{i}^{std}$ 时，该项为 `0.75`。 |
