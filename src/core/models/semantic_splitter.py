...
def trans2numpy(params):
    if isinstance(params,list):
        return np.array(params)
    return params

class SemanticParagraphSplitter:
    def __init__(self, model, buffer_size=1, threshold=80):
    ...
    def build_sentences_dict(self, sentences):
        ...
        embeddings = self.model.get_embeddings_batch(
    ...
    def calculate_cosine_distances(sentences: list):
        ...
        for i in range(len(sentences) - 1):
            embedding_current = trans2numpy(sentences[i]["combined_sentence_embedding"])
            embedding_next = trans2numpy(sentences[i + 1]["combined_sentence_embedding"])
