...
def get_AZ_name(data):
    ...
    for i,item in enumerate(data):
        ...
        if re.search(r"^N[Oo]\.[0-9]{6}",content) and left == 0:
            left = i
...
def combined_text(params):
    ...
    while i < len(params) - 1:
        ...
        if head["content"].endswith("。") or head["content"].endswith("xlsx") or re.search(
            ...
            tail = params[i + 1]
            if re.search(
                    r"(^第[一二三四五六七]节)|(^[0-9]{1})|(^单位：)|(^单位:)|(^□适用)|(^图注)|(^√适用)|(^第[一二三四五六七]部分)|(^（[一二三四五六七]）)|(^[一二三四五六七]、)|(^□是)|(^√是)|(^\([一二三四五六七]\))|(^[一二三四五六七])",
                    tail["content"]):
                result.append(head)
                i += 1
                continue
...
def merge_sub_chunck(file,index,details):
    ...
    if file.startswith("AF"):
        ...
        from src.core.docx_parser.docx_operation import OperationAW
        ope_aw = OperationAW()
        ...
        from src.core.utils.cleaner import filter_redundancy
        from src.core.docx_parser.docx_operation import OperationAY
        ope_ay = OperationAY()
        ...
        from src.core.docx_parser.docx_operation import OperationAZ
        ope_az = OperationAZ()
        chunk_result = details[index + 1:]
